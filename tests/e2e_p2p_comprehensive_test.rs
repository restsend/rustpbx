...
async fn test_p2p_basic_call_flow_with_cdr() {
    ...
    let pbx = TestPbx::start_with_inject(portpicker::pick_unused_port().unwrap(), TestPbxInject {
        callrecord_sender: Some(cdr_sender), ..Default::default()
    }).await;
    ...
    rwi.send(&serde_json::json!({"rwi":"1.0","action":"call.originate","action_id":"o1","params":{
    ...
    rwi.assert_event_sequence(&["call_ringing","call_early_media","call_answered"]).await;
    ...
    assert_eq!(r.hangup_reason, Some(rustpbx::callrecord::CallRecordHangupReason::BySystem));
    assert!((r.end_time - r.start_time).num_seconds() >= 0);
    eprintln!("[P2P] ✓ flow 180→183→200→BYE, CDR: status={} dur={}s", r.details.status, (r.end_time-r.start_time).num_seconds());
...
async fn test_p2p_call_reject_486_with_cdr() {
    ...
    rwi.send(&serde_json::json!({"rwi":"1.0","action":"call.originate","action_id":"o2","params":{
        "call_id":call_id,"destination":bob.sip_uri("bob"),
        "caller_id":format!("sip:caller@{}",pbx.sip_host()),
        "context":"default","timeout_secs":15
    }})).await;
    ...
    if let Some(r) = cdr.wait_for_record(&call_id,2).await {
        ...
        eprintln!("[P2P] ✓ CDR: status={} code={} hangup={:?}", r.details.status, r.status_code, r.hangup_reason);
    } else {
        eprintln!("[P2P] ⚠ No CDR for rejected call (expected: RWI path may skip non-answered calls)");
    }
...
async fn test_p2p_no_answer_timeout() {
    ...
    rwi.send(&serde_json::json!({"rwi":"1.0","action":"call.originate","action_id":"o3","params":{
        "call_id":call_id,"destination":bob.sip_uri("bob"),
        "caller_id":format!("sip:caller@{}",pbx.sip_host()),
    ...
    assert!(no_answer.is_some(), "should receive call_no_answer after timeout");
    eprintln!("[P2P] ✓ No answer timeout → call_no_answer");

    sleep(Duration::from_millis(500)).await;
    let r = cdr.wait_for_record(&call_id,3).await.expect("CDR for no-answer");
...
async fn test_p2p_cancel_during_ringing() {
    ...
    rwi.send(&serde_json::json!({"rwi":"1.0","action":"call.originate","action_id":"o4","params":{
        "call_id":call_id,"destination":bob.sip_uri("bob"),
        "caller_id":format!("sip:caller@{}",pbx.sip_host()),
        "context":"default","timeout_secs":15
    }})).await;
...
async fn test_p2p_correct_caller_callee_in_cdr() {
    ...
    let bob = TestUa::callee_with_username(portpicker::pick_unused_port().unwrap(),1,"bob42").await;
    let call_id = format!("p2p-cdr-{}", Uuid::new_v4());
    let caller_id = format!("sip:alice-smith@{}", pbx.sip_host());

    let mut rwi = RwiCollector::connect(&pbx.rwi_url, TEST_TOKEN).await;
    rwi.wait_for_event_type("command_completed",3).await;
    rwi.send(&serde_json::json!({"rwi":"1.0","action":"call.originate","action_id":"o5","params":{
    ...
    let r = cdr.wait_for_record(&call_id,3).await.expect("CDR");
    cdr.assert_cdr(&r, &CdrExpectation::default()
    ...
    eprintln!("[P2P] ✓ CDR caller=alice-smith callee=bob42 duration={}s", (r.end_time-r.start_time).num_seconds());
