...
async fn test_e2e_webrtc_caller_to_rtp_callee_via_bridge() {
    ...
    assert!(
        !bridge_rtp_sdp.contains("fingerprint"),
        "Bridge RTP offer must not have fingerprint"
    );
    ...
    assert!(
        ...
        "Answer MUST NOT have setup:actpass (was causing 488)"
    );
    assert!(
...
async fn test_e2e_rtp_caller_to_webrtc_callee_via_bridge() {
    ...
    let caller_offer = caller.local_description().await.unwrap();
    assert!(
        caller_offer.contains("RTP/AVP"),
        "RTP caller offer must be plain RTP"
    );
    ...
    assert!(
        bridge_webrtc_sdp.contains("fingerprint"),
        "Bridge WebRTC offer must have fingerprint"
    );
    ...
    assert!(
        ...
        "RTP answer must not have DTLS fingerprint"
    );
    assert!(
        !answer_sdp.contains("ice-ufrag"),
...
a=sendrecv\r\n";
    ...
    assert!(
        webrtc_sdp.contains("setup:passive"),
...
async fn test_e2e_webrtc_caller_rtp_callee_pcmu_only_allow_codecs() {
    ...
    let caller_offer = caller.local_description().await.unwrap();
    assert!(
        caller_offer.contains("opus"),
        "Caller must offer Opus for this test"
    );
    ...
    assert!(
        ...
        "RTP side must NOT offer Opus (filtered by allow_codecs)"
    );
    ...
    assert!(
        answer_sdp.contains("fingerprint:sha-256"),
        "Must have real fingerprint"
    );
    assert!(
        !answer_sdp.contains("setup:actpass"),
        "Must NOT have actpass"
    );
...
async fn test_e2e_rtp_caller_g729_dropped_on_webrtc_side() {
    ...
    let caller_offer = caller.local_description().await.unwrap();
    assert!(
        caller_offer.contains("G729"),
        "Caller must offer G729 for this test"
    );
    ...
    assert!(
        bridge_webrtc_sdp.contains("PCMU"),
        "WebRTC side must offer PCMU as fallback"
    );
    ...
    assert!(
        !rtp_answer_sdp.contains("fingerprint"),
...
async fn test_e2e_no_transcode_path_same_codec_on_both_sides() {
    ...
    let caller_first = codec_lists
        .caller_side
        .iter()
        .find(|c| !c.is_dtmf())
        .unwrap();
    let callee_first = codec_lists
        .callee_side
        .iter()
        .find(|c| !c.is_dtmf())
        .unwrap();
    ...
    assert_eq!(
        caller_first.codec,
        CodecType::Opus,
        "Caller side first codec must be Opus"
    );
    assert_eq!(
        callee_first.codec,
        CodecType::Opus,
        "Callee side first codec must be Opus"
    );
    assert_eq!(
        caller_first.codec, callee_first.codec,
...
async fn test_e2e_caller_payload_types_preserved() {
    ...
    let caller_pcmu = codec_lists
        .caller_side
        .iter()
        .find(|c| c.codec == CodecType::PCMU)
        .unwrap();
    let caller_pcma = codec_lists
        .caller_side
        .iter()
        .find(|c| c.codec == CodecType::PCMA)
        .unwrap();

    assert_eq!(
        caller_pcmu.payload_type, 0,
        "PCMU PT must be 0 (from caller SDP)"
    );
    assert_eq!(
        caller_pcma.payload_type, 8,
        "PCMA PT must be 8 (from caller SDP)"
    );

    // Callee side should use standard PTs
    let callee_pcmu = codec_lists
        .callee_side
        .iter()
        .find(|c| c.codec == CodecType::PCMU)
        .unwrap();
    let callee_pcma = codec_lists
        .callee_side
        .iter()
        .find(|c| c.codec == CodecType::PCMA)
        .unwrap();
    ...
    assert_eq!(
        ...
        "Callee PCMU uses standard PT 0"
    );
    assert_eq!(
        callee_pcma.payload_type, 8,
        "Callee PCMA uses standard PT 8"
    );
}
...
a=rtpmap:101 telephone-event/8000\r\n";
    ...
    let callee_audio: Vec<_> = codec_lists
        .callee_side
        .iter()
        .filter(|c| !c.is_dtmf())
        .collect();
    ...
    assert_eq!(
        ...
        CodecType::G722,
        "G722 first per allow_codecs"
    );
    assert_eq!(
        callee_audio[1].codec,
        CodecType::PCMU,
        "PCMU second per allow_codecs"
    );
    assert_eq!(
        callee_audio[2].codec,
        CodecType::PCMA,
        "PCMA third per allow_codecs"
    );
}
...
async fn test_e2e_early_media_then_200_ok_same_sdp_rtp_flow_continues() {
    ...
    let recv_handle = tokio::spawn(async move {
        ...
        loop {
            match tokio::time::timeout(std::time::Duration::from_millis(500), track.recv()).await {
