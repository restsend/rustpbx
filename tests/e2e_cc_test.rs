...
async fn test_cc_agent_callout_cdr() {
    ...
    rwi.send(&serde_json::json!({"rwi":"1.0","action":"call.originate","action_id":"cc-orig","params":{
    ...
    eprintln!("[CC] ✓ Agent callout CDR: status={} dur={}s", r.details.status, (r.end_time-r.start_time).num_seconds());
