...
async fn test_queue_cdr_generation() {
    ...
    let pbx = TestPbx::start_with_inject(sp, TestPbxInject {
        ...
        routes: Some(vec![rustpbx::proxy::routing::RouteRule{
            name:"to-q".into(),priority:10,
            match_conditions:rustpbx::proxy::routing::MatchConditions{
                to_user:Some("q".into()),..Default::default()},
            action:rustpbx::proxy::routing::RouteAction{
                queue:Some("q".into()),..Default::default()},..Default::default()
        }]),
        queues: Some(HashMap::from([("q".to_string(),
            rustpbx::proxy::routing::RouteQueueConfig{
                name:Some("q".into()),accept_immediately:true,
        ...
        callrecord_sender: Some(cs), ..Default::default()
    }).await;

    let agent = TestUa::callee_with_username(ap, 1, "agent").await;
    let _caller = TestUa::caller_with_target(cp, "caller",
        format!("sip:q@127.0.0.1:{}",sp)).await;

    sleep(Duration::from_secs(8)).await;
    assert!(!agent.has_rtp_rx() || !agent.has_rtp_tx(),
    ...
    if !all.is_empty() {
        eprintln!("[QUEUE] ✓ CDR from Drop safety net: {} record(s)", all.len());
        for r in &all {
...
async fn test_queue_sequential_two_agents() {
    ...
    let pbx = TestPbx::start_with_inject(sp, TestPbxInject {
        ...
        routes: Some(vec![rustpbx::proxy::routing::RouteRule{
            name:"to-sq".into(),priority:10,
            match_conditions:rustpbx::proxy::routing::MatchConditions{
                to_user:Some("sq".into()),..Default::default()},
            action:rustpbx::proxy::routing::RouteAction{
                queue:Some("sq".into()),..Default::default()},..Default::default()
        }]),
        queues: Some(HashMap::from([("sq".to_string(),
            rustpbx::proxy::routing::RouteQueueConfig{
                name:Some("sq".into()),accept_immediately:true,
                strategy:rustpbx::proxy::routing::RouteQueueStrategyConfig{
                    mode:rustpbx::proxy::routing::QueueDialMode::Sequential,
                    wait_timeout_secs:Some(10),
                    targets:vec![
                        rustpbx::proxy::routing::RouteQueueTargetConfig{
                            uri:format!("sip:a1@127.0.0.1:{}",a1p),label:Some("A1".into())},
                        rustpbx::proxy::routing::RouteQueueTargetConfig{
                            uri:format!("sip:a2@127.0.0.1:{}",a2p),label:Some("A2".into())},
                    ],
                },..Default::default()
            })])),
        ..Default::default()
    ...
    let _caller = TestUa::caller_with_target(cp, "caller",
        format!("sip:sq@127.0.0.1:{}",sp)).await;
