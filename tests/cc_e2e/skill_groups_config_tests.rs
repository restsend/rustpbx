...
/// `skill_groups_combined_file` resolves to `<tempdir>/config/cc/skill_groups/skill_groups.generated.toml`
/// when `config_path = "<tempdir>/config.toml"` (default `proxy.generated_dir = ./config`).
...
async fn test_reload_inserts_new_groups_from_toml() {
    ...
    let sg_dir = generated_cc_dir(&tmp).join("skill_groups");
...
/// Write a file into `<tmp>/config/cc/skill_groups/` and return the directory path.
fn write_sg_file(tmp: &TempDir, name: &str, content: &str) -> std::path::PathBuf {
    let dir = generated_cc_dir(tmp).join("skill_groups");
...
"#;
    ...
    let cache =
        rustpbx::addons::cc::CcAddonState::load_skill_groups_from_dir(Some(&config_path_for(&tmp)))
            .await;
