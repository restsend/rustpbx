...
#[tokio::test]
async fn test_basic_p2p_call_establishes_and_tears_down() -> Result<()> {
    ...
    assert!(answered, "Bob should answer the call");

    let alice_id = match tokio::time::timeout(Duration::from_secs(5), caller).await {
        Ok(Ok(Ok(id))) => Some(id),
        _ => None,
    };
    assert!(alice_id.is_some(), "Call should be established");
