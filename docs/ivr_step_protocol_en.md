...
| `POST {url}/end` | Session ends | ❌ fire-and-forget (body: `{"reason": "..."}`) |
...
| `error` | `reason: string` | An execution error occurred |
...
**`POST {url}/end`** (session end) — Optional cleanup. No response expected.
- Body: `{"reason": "Normal"}` (or `"Transfer(2001)"`, `"Hangup"`, `"Error(...)"`).
- Fire-and-forget: no retry, response ignored.
