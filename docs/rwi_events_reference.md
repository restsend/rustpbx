...
| `event` | object | 完整事件载荷，键为 event_type |
...
| `ani` | Option\<String\> | 主叫号码（标准化纯号码） |
| `dnis` | Option\<String\> | 被叫号码 / DNIS |
...
| `caller` | String | 主叫 |
| `callee` | String | 被叫 |
| `timestamp` | String | 事件记录时间（ISO UTC） |
| `step_index` | u32 | 步骤序号 |
...
| `duration_ms` | u64 | Provider HTTP 调用耗时（毫秒） |
...
| `step_execute_duration` | Option\<u64\> | execute_node 动作执行耗时（毫秒）。仅步骤执行完成时有值；WaitFor 时为 null |
| `extra` | Option\<JSON Object\> | Provider 透传的额外数据。Provider 在每次响应的 ActionNode.extra 中返回完整对象，RustPBX 透传存储并原样输出 |

> **时间字段说明**：
> - `timestamp` — trace 记录的当前时间
> - `step_start_time` — 当前步骤的开始时间（上一步结束或 session 开始）
> - `step_end_time` — 步骤结束时间（仅完成时）
>
> **耗时字段说明**：
> - `duration_ms` — 每次请求 provider 的 HTTP 往返耗时，始终有值
> - `step_execute_duration` — execute_node 执行动作（播放音频、收集 DTMF 等）的耗时，仅步骤完成时有值
...
| `third_party_dn` | Option\<String\> | 第三方 DN |
| `vq_name` | Option\<String\> | 虚拟队列名 |
| `routing_target` | Option\<String\> | 路由目标 |
| `skill_group` | Option\<String\> | 技能组 |
| `target_dn` | Option\<String\> | 目标 DN |
