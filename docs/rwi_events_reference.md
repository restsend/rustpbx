...
Step-Mode IVR 调试跟踪，每一步 provider 往返产生一条。
...
| `step_index` | u32 | 步骤序号 |
| `event_type` | String | 事件类型（如 `provider_response`） |
| `action_type` | String | 动作类型（如 `Transfer`、`Prompt`） |
| `action_json` | Option\<String\> | 动作详情 JSON |
| `result_kind` | String | 结果类型（`terminal`、`continue` 等） |
...
| 74 | AGENTLOGOUT | 坐席登出 |
| 75 | AGENTREADY | 坐席就绪 |
| 76 | AGENTNOTREADY | 坐示未就绪 |
| 87 | ONHOOK | 摘机 |
| 102 | REMOTECONNECTIONFAILED | 远程连接失败 |
