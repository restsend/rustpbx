...
pub fn inject_audio(
    engine: &crate::engine::MediaEngine,
    session_id: &str,
    path: &str,
    target: crate::engine::command::InjectTarget,
    mute_peer: bool,
) -> Result<()> {
    info!(session_id, path, ?target, "InjectAudio: playing file");
    engine.send(crate::engine::MediaCommand::InjectAudio {
        session_id: session_id.to_string(),
        source: crate::engine::command::PlaySource::File {
            path: path.to_string(),
        },
        target,
        mute_peer,
