...
mod media_engine_tests {
    use crate::LegId;
    use crate::engine::MediaCommand;
    use crate::engine::event::MediaEvent;
    use crate::media_stream::TrackMap;
    ...
    fn test_media_command_session_id() {
        ...
        let cmd = MediaCommand::Play {
            session_id: "s1".into(),
            leg_id: None,
            source: crate::engine::PlaySource::Silence,
    ...
    async fn test_track_map_remove() {
        ...
        impl crate::Track for DummyTrack {
            fn id(&self) -> &str {
                &self.0
            }
            async fn handshake(
        ...
        map.insert(
            ...
            std::sync::Arc::new(tokio::sync::Mutex::new(
                Box::new(DummyTrack("key".into())) as Box<dyn crate::Track>
            )),
        );
        assert_eq!(map.len(), 1);
        map.remove("key");
    ...
    fn bench_engine() -> (
        ...
        use crate::engine::{MediaEngine, MediaEngineConfig};
        ...
        for i in 0..n {
            engine
                .send_async(MediaCommand::AddLeg {
                    session_id: "bench-sess".into(),
                    leg_id: format!("leg-{}", i),
                    transport: crate::engine::LegTransport::File {
                        path: "/dev/null".into(),
                    },
                    codec_profile: Some(crate::engine::CodecProfile::pcmu()),
                })
                .await
                .unwrap();
        }
    ...
    fn bench_dtmf_detection() {
        let mut detector = crate::dtmf::DtmfDetector::default();
        let n = 100_000usize;
        let start = std::time::Instant::now();
        for i in 0..n {
