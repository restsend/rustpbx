...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        if !manager.has_column(table_name, "outbound_sip_trunk_id").await? {
            manager
                .alter_table(
                    sea_query::Table::alter()
                        .table(CallRecordEntity)
                        .add_column(ColumnDef::new(CallRecordColumn::OutboundSipTrunkId).big_integer().null())
                        .to_owned(),
                )
                .await?;
        }
        ...
        if !manager.has_index(table_name, "idx_call_records_outbound_sip_trunk_id").await? {
            manager
                .create_index(
