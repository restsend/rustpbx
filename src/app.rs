...
impl AppStateBuilder {
    ...
    pub async fn build(self) -> Result<AppState> {
        ...
        // Stamp this node's cluster-internal IP onto every RWI event as `node_ip`
        ...
        if let Some(ref gw) = core.rwi_gateway {
            let node_ip = sip_server
                .inner
                .cluster_self_addr
                .as_ref()
                .map(|addr| addr.addr.host.to_string());
            gw.write().set_node_ip(node_ip);
        }
    ...
    let response = match config.browser_ice_servers(now) {
        ...
        Err(error) => {
            ...
            (
                ...
                "Invalid TURN REST configuration",
            )
                .into_response()
        }
    };
...
mod ice_servers_tests {
    ...
    async fn test_turn_rest_ice_servers_http_response() {
        ...
        let app = Router::new().route(
            "/iceservers",
            get(iceservers_handler).with_state(Arc::new(config)),
        );
        let response = app
            .oneshot(
                Request::builder()
