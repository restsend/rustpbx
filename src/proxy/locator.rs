...
impl Locator for MemoryLocator {
    ...
    /// Periodically sweep expired registrations that would otherwise leak —                                                                                                                                                                       
        ...
        self.locations.retain(|_, map| {
            map.retain(|_, loc| {
                if loc.is_expired_at(now) {
                    info!(binding = %loc.binding_key(), "swept expired registration");
...
mod tests {
    ...
    #[tokio::test]
    async fn memory_locator_same_instance_id_updates_old_destination() {
        let locator = MemoryLocator::new();
        ...
        // Lookup with old AoR must find the old location with updated destination
    ...
    #[tokio::test]
    async fn memory_locator_same_instance_id_does_not_update_expired_location() {
        let locator = MemoryLocator::new();
